...
function Menu(menuItemsArray: MenuItem[][], isDotMenu?: boolean, userData?: User) {
  return (
    <div className={shadowedWhiteColor + " select-none rounded-md absolute px-0 py-1 right-5 top-8 min-w-52 border border-white dark:border-neutral-600"}>
      { isDotMenu && userData ?
        <div className="mx-3 mt-3 mb-0.5 p-4 pr-8 bg-purple-tint rounded">
          <h2 className="text-purple">{userData.firstName + " " + userData.lastName}</h2>
          <p className="text-purple text-sm">{userData.email}</p>
        </div> : null
      }
      {
