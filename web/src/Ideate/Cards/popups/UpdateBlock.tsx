...
import { Heading, Modal, Popup, Text } from './NewBlock';
...
type Props = {
    ...
    title: string,
    text: string,
    updateFile: (title: string, text: string) => void,
    closePopup: () => void
}
...
const UpdateBlock = (props: Props) => {
    ...
    const updateBlock = (event: FormEvent) => {
        ...
        try {
            if (formData.title === '') throw("Please enter a title");
            if (formData.text === '') throw("Please enter some text");
            props.updateFile(formData.title, formData.text);
        }
        catch (error) {
            // @ts-ignore
            setError(error);
        ...
        <Modal onClick={props.closePopup}>
            <div onClick={(e) => e.stopPropagation()}>
                ...
                <Popup onSubmit={updateBlock} className={getClassCode("", props.isDarkTheme)}>
                    <Heading id="title" className={props.color + "-color"} >{props.title}</Heading>
